...
public class MemberService {
    ...
    public MemberResponse getMember(Long id){
        Member member = jpaMemberRepository.findById(id).orElseThrow(()-> new IllegalArgumentException(("존재하지 않는 멤버입니다.")));

        return MemberResponse.from(member);
    }

    public void updateMember(Long id, UpdateMemberRequest updateMemberRequest) {
        Member member= jpaMemberRepository.findById(id).orElseThrow(()-> new IllegalArgumentException("존재하지 않는 회원입니다."));
